...
class MyBuffer(Buffer):
    ...
    def add_data(self, examples, labels=None, logits=None, task_labels=None, attention_maps=None, update_strategy=1,
    ...
    def buffer_update_fn(self, data_buffer, label_buffer, logits_buffer, data, labels, logits, distance_method):
        ...
        # 计算每个标签的目标数量
        unique_labels, label_counts = combined_labels.unique(return_counts=True)
        target_label_count = (self.buffer_size + len(unique_labels) - 1) // len(unique_labels)

        # 创建标签到索引的映射
        label_to_indices = {label.item(): (combined_labels == label).nonzero(as_tuple=True)[0] for label in
                            unique_labels}
        ...
        for label in unique_labels:
            ...
            if len(indices) > target_label_count:
                ...
                indices_to_keep = torch.tensor([i for i in range(len(indices)) if i not in indices_to_remove],
                                               device=self.device)
            else:
...
#         # 加载预训练的 ResNet-18 模型
#         pretrained_model = models.resnet18(pretrained=True)
#         feature_extractor = FeatureExtractor(pretrained_model, input_channels=1)  # 确保输入通道为1
...
#     def add_data(self, examples, labels=None, logits=None, task_labels=None, attention_maps=None, update_strategy=1,
#                  distance_method='cosine'):
#         print("adding data to the buffer...")
#         if not hasattr(self, 'examples'):
#             self.init_tensors(examples, labels, logits, task_labels)
#
#         if self.examples is None:
...
#                 self.task_labels = torch.cat((self.task_labels, task_labels.to(self.device)), dim=0)
#         else:
#             self.examples, self.labels = self.buffer_update_fn(self.examples, self.labels, examples, labels,
#                                                                distance_method)
#         self.num_seen_examples += len(examples)
#
#     def buffer_update_fn(self, data_buffer, label_buffer, batch_X, batch_y, distance_method):
#         # 合并当前buffer和新batch的数据
#         combined_data = torch.cat((data_buffer, batch_X), dim=0)
#         combined_labels = torch.cat((label_buffer, batch_y), dim=0)
#
#         # 计算每个标签的目标数量
#         unique_labels, label_counts = combined_labels.unique(return_counts=True)
#         target_label_count = (self.buffer_size + len(unique_labels) - 1) // len(unique_labels)
...
#         updated_label_buffer = combined_labels[selected_indices]
#
#         return updated_data_buffer, updated_label_buffer
#
#     def get_features(self, samples):
#         # 使用预训练模型提取样本特征
#         with torch.no_grad():
...
#         return features
#
#     # def compute_average_cosine_distances(self, features, device, block_size=200):
#     #     features = features.to(device)
#     #     n, m = features.shape
#     #     avg_cosine_divergence = torch.zeros(n, device=device)
#     #
#     #     for i in range(0, n, block_size):
#     #         end_i = min(i + block_size, n)
#     #         features_i = features[i:end_i]
#     #
#     #         for j in range(0, n, block_size):
#     #             end_j = min(j + block_size, n)
#     #             features_j = features[j:end_j]
#     #
#     #             cosine_similarity_block = F.cosine_similarity(features_i.unsqueeze(1), features_j.unsqueeze(0), dim=2)
#     #             cosine_distance_block = 1 - cosine_similarity_block
#     #
#     #             if i == j:
#     #                 mask = torch.eye(end_i - i, device=device).bool()
#     #                 cosine_distance_block.masked_fill_(mask, 0)
#     #
#     #             avg_cosine_divergence[i:end_i] += cosine_distance_block.sum(dim=1)
#     #
#     #     avg_cosine_divergence /= (n - 1)
#     #     return avg_cosine_divergence
#
#     def compute_average_cosine_distances(self, features, device):
#         features = features.to(device)
#         n, m = features.shape
#         cosine_similarity_matrix = F.cosine_similarity(features.unsqueeze(1), features.unsqueeze(0), dim=2)
#         cosine_distance_matrix = 1 - cosine_similarity_matrix
#         mask = torch.eye(n, device=device).bool()
#         cosine_distance_matrix.masked_fill_(mask, 0)
#         avg_cosine_divergence = cosine_distance_matrix.sum(dim=1) / (n - 1)
#         return avg_cosine_divergence
#
#     def compute_average_js_divergences(self, features, device):
#         epsilon = 1e-10
